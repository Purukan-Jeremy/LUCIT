...
const SlideTransition: React.FC<SlideTransitionProps> = ({ children }) => {
  ...
  const [prevLocation, setPrevLocation] = useState<typeof location | null>(
    null,
  );
  const [isAnimating, setIsAnimating] = useState(false);
  const [direction, setDirection] = useState<"to-history" | "to-analysis">(
    "to-history",
  );

  useEffect(() => {
    const isToHistory =
      displayLocation.pathname === "/analysis" &&
      location.pathname === "/history";
    const isToAnalysis =
      displayLocation.pathname === "/history" &&
  ...
  const renderRoutes = (loc: typeof location) => {
    return React.cloneElement(
      children as React.ReactElement<{ location?: unknown }>,
      { location: loc },
    );
  };

  const isParallelRoute =
    (location.pathname === "/analysis" || location.pathname === "/history") &&
    (displayLocation.pathname === "/analysis" ||
      displayLocation.pathname === "/history");
