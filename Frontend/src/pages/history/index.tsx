...
const DonutChart = ({ percent, label, color }: { percent: number, label: string, color: string }) => {
  const radius = 35;
...
function HistoryPage() {
  ...
  return (
    <main className="history-page-v2">
      <div className="history-container">
        <header className="history-header-v2">
          <div className="history-header-left">
            ...
            <span className="history-breadcrumb">LUCIT / Clinical Archives</span>
        ...
        <section className="history-content-v2">
          {isLoading ? (
            ...
            <div className="history-error-v2">
              ...
              <p>{error}</p>
              <button onClick={() => fetchHistory(query)} className="retry-btn">Retry Connection</button>
            </div>
          ) : items.length === 0 ? (
                    ...
                    <div className="history-item-main" onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}>
                      ...
                      <div className="item-info">
                        ...
                        <h3 className="item-prediction">
                          {isSegmentation ? "" : formatPredictionLabel(item.prediction)}
                        </h3>
                        <div className="item-tags">
                          <span className={`type-badge ${item.model.toLowerCase()}`}>{item.model}</span>
                          <span className="pathologist-badge">Dr. {userFullname.split(' ')[0]}</span>
                        </div>
                      </div>
                      ...
                      <div className="item-visual-summary">
                        <DonutChart 
                          ...
                          color={isSegmentation ? "#d32f2f" : getConfidenceColor(confValue)}
                        />
                      </div>
                      ...
                      <div className="item-actions-v2">
                        ...
                        <div className={`expand-chevron ${expandedId === item.id ? 'active' : ''}`}>
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="m6 9 6 6 6-6"/>
                          </svg>
                        </div>
                      ...
                      <div className="history-item-details">
                        <div className="details-grid">
                          ...
                          <div className="detail-findings">
                            <h4 className="findings-title">Clinical Findings & AI Interpretation</h4>
      ...
      {deleteTargetId !== null && (
        <div className="medical-modal-overlay" onClick={() => setDeleteTargetId(null)}>
          <div className="medical-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body-v2">
              <p>You are about to permanently remove <strong>Exam #{deleteTargetId}</strong> from the clinical archives.</p>
              <p className="modal-subtext">This action cannot be undone and will remove all associated AI diagnostic data.</p>
            </div>
            <div className="modal-footer-v2">
              <button className="modal-btn-cancel" onClick={() => setDeleteTargetId(null)}>Cancel</button>
              <button className="modal-btn-confirm" onClick={handleConfirmDelete}>Confirm Delete</button>
            </div>
          </div>
        </div>
      )}
