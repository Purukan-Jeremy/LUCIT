...
class ImageClassifier:
    ...
    def load_model(model_name="mobilenetv2.keras"):
        ...
        """
        if model_name not in ImageClassifier._model_cache:
            ...
            ImageClassifier._model_cache[model_name] = tf.keras.models.load_model(
                str(resolved_model_path),
                compile=False,
            )
            print(f"Model loaded successfully from: {resolved_model_path}")

        return ImageClassifier._model_cache[model_name]
    ...
    def clear_model_cache(model_name=None):
        ...
        """
        if model_name:
            ImageClassifier._model_cache.pop(model_name, None)
        else:
            ImageClassifier._model_cache = {}
...
=======
def load_model(model_name="mobilenetv2.keras"):
    return ImageClassifier.load_model(model_name)


def clear_model_cache(model_name=None):
    ImageClassifier.clear_model_cache(model_name)


ClassificationModelManager = ImageClassifier
>>>>>>> Stashed changes
